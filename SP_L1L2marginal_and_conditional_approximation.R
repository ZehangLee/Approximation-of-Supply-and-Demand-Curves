...
#### 1.1.1  Approximation with 50 nodes ##### 
...
#
Approx = foreach(i=1:max(day_ahead_supply$time),.combine='rbind', .options.snow=opts,.packages = c("cubature","nor1mix","mixtools")) %dopar% {
...
save_path = paste("C:/Zehang_workspace/Approximation_Supply_Demand/gamma weight approximation/SP_Day-ahead_Supply_L2_Marginal_GammaWeight_Approx_",n_node,"nodes",".RData",sep = "")
...
#### 1.2 L2 + conditional distribution ####
...
plot(ecdf(day_ahead_supply$p), main = "Conditional Empirical distributions")
lines(ecdf(day_ahead_supply$p[day_ahead_supply$q_scaled>Q]), col = "blue")
legend("bottomright",legend=c("Original", paste0("Q=",round(Q,digits = 8))),
       col=c("black", "blue"), lty=1,cex =1.2, box.lty=2, box.lwd=2)




##### 1.2.1  Approximation with 50 nodes ##### 
n_node = 50
probabilities= seq(0, 1, length.out = n_node)
...
save_path = paste("C:/Zehang_workspace/Approximation_Supply_Demand/gamma weight approximation/SP_Day-ahead_Supply_L2_Conditional_GammaWeight_Approx_",n_node,"nodes",".RData",sep = "")
save(Approx,file=save_path)
...
#### 2.1.1  Approximation with 50 nodes ##### 
...
opts = list(progress=progress)
#
L1_Approx_marginal_50nodes = foreach(i=1:max(day_ahead_supply$time),.combine='c', .options.snow=opts,.packages = c("cubature","nor1mix")) %dopar% {
...
save_path = paste("C:/Zehang_workspace/Approximation_Supply_Demand/gamma weight approximation/sp_day-ahead_supply_L1_marginal_GammaWeight_Approx_",n_node,"nodes",".RData",sep = "")
save(L1_Approx_marginal_50nodes,file=save_path)
...
cp2 = sort(unique(c(boundaries)))
cp2 = cp2[2:(length(cp2)-1)]
cq2 = L1_Approx_marginal_50nodes[[i]][,3]
...
#### 2.2 L2 + conditional distribution ####
...
##### 2.2.1  Approximation with 50 nodes ##### 
n_node = 50
probabilities= seq(0, 1, length.out = n_node+1)
node = quantile(day_ahead_supply[day_ahead_supply$q_scaled > Q,]$p_scaled,probabilities,type = 1)
boundaries = cbind(node[1:50],node[2:51])
...
opts = list(progress=progress)
#
L1_Approx_conditional_50nodes = foreach(i=1:max(day_ahead_supply$time),.combine='c', .options.snow=opts,.packages = c("cubature","nor1mix")) %dopar% {
...
cp2 = sort(unique(c(boundaries)))
cp2 = cp2[2:(length(cp2)-1)]
cq2 = L1_Approx_conditional_50nodes[[i]][,3]
...
save_path = paste("C:/Zehang_workspace/Approximation_Supply_Demand/gamma weight approximation/sp_day-ahead_supply_L1_conditional_GammaWeight_Approx_",n_node,"nodes",".RData",sep = "")
save(L1_Approx_conditional_50nodes,file=save_path)




#############################################################################
#                    3. L2 Approximation of demand curves                   #
#############################################################################
...
#### 3.1 L2 + marginal ####
##### 3.1.1  Approximation with 50 nodes ##### 
...
save_path = paste("C:/Zehang_workspace/Approximation_Supply_Demand/gamma weight approximation/sp_day-ahead_demand_L2_marginal_GammaWeight_Approx_",n_node,"nodes",".RData",sep = "")
save(Approx,file=save_path)



#### 3.2 L2 + conditional distribution ####
...
plot(ecdf(day_ahead_demand$p), main = "Conditional Empirical distributions")
lines(ecdf(day_ahead_demand$p[day_ahead_demand$q_scaled>Q]), col = "blue")
legend("bottomright",c("Original",  paste0("Q=",round(Q,digits = 8))),
       col=c("black", "blue"), lty=1,cex =1.2, box.lty=2, box.lwd=2)



##### 3.2.1  Approximation with 50 nodes ##### 
n_node = 50
probabilities= seq(0, 1, length.out = n_node)
...
save_path = paste("C:/Zehang_workspace/Approximation_Supply_Demand/gamma weight approximation/sp_day-ahead_demand_L2_conditional_GammaWeight_Approx_",n_node,"nodes",".RData",sep = "")
save(Approx,file=save_path)
...
#### 4.1.1  Approximation with 50 nodes ##### 
...
opts = list(progress=progress)
#
L1_Approx_marginal_50nodes = foreach(i=1:max(day_ahead_demand$time),.combine='c', .options.snow=opts,.packages = c("cubature","nor1mix")) %dopar% {
...
save_path = paste("C:/Zehang_workspace/Approximation_Supply_Demand/gamma weight approximation/sp_day-ahead_demand_L1_marginal_GammaWeight_Approx_",n_node,"nodes",".RData",sep = "")
save(L1_Approx_marginal_50nodes,file=save_path)
...
p2 = p2[2:(length(p2)-1)]
q2 = L1_Approx_marginal_50nodes[[i]][,3]
...
#### 4.2.1  Approximation with 50 nodes ##### 
n_node = 50
probabilities= seq(0, 1, length.out = n_node+1)
node = quantile(day_ahead_demand[day_ahead_demand$q_scaled > Q,]$p_scaled,probabilities,type = 1)
boundaries = cbind(node[1:50],node[2:51])
...
opts = list(progress=progress)
#
L1_Approx_conditional_50nodes = foreach(i=1:max(day_ahead_demand$time),.combine='c', .options.snow=opts,.packages = c("cubature","nor1mix")) %dopar% {
...
save_path = paste("C:/Zehang_workspace/Approximation_Supply_Demand/gamma weight approximation/sp_day-ahead_demand_L1_conditional_GammaWeight_Approx_",n_node,"nodes",".RData",sep = "")
save(L1_Approx_conditional_50nodes,file=save_path)
...
p2 = sort(unique(c(boundaries)))
p2 = p2[2:(length(p2)-1)]
q2 = L1_Approx_conditional_50nodes[[i]][,3]
